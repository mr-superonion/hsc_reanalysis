...
def sample_cov(xi_arrays, mode="full"):
    ...
    for i in range(npoints):
        for j in range(npoints):
            Cov[i, j] = (
                np.sum((xi_arrays[:, i] - xi_mean[i]) * (xi_arrays[:, j] - xi_mean[j]))
                / nsample
            )
...
class NumberDensity(object):
    ...
    def from_fits(cls, extension):
        ...
        if all(x is None for x in sigma_e):
            ...
            assert not any(
                x is None for x in sigma_e
            ), "Please specify all or none of the SIG_E_ in your n(z) section {}".format(
                extension.name
            )
        ...
        if all(x is None for x in ngal):
            ...
            assert not any(
                x is None for x in ngal
            ), "Please specify all or none of the NGAL in your n(z) section {}".format(
                extension.name
            )
            ngal = np.array(ngal)
...
class SpectrumMeasurement(object):
    ...
    def convert_angular_units(self, unit):
        ...
        print(
            "Converting angle units of {} from {} -> {} (factor {})".format(
    ...
    def cut_bin_pair(self, bin_pair, complain=False):
        ...
        if (mask.sum() == 0) and complain:
            raise ValueError(
                """You tried to cut bin pair %d,%d from spectrum named %s
    ...
    def to_fits(self):
        ...
        if self.extra_cols is not None:
            for (colname, arr) in self.extra_cols.iteritems():
                columns.append(
                    fits.Column(name="XTRA_" + colname, array=arr, format="D")
                )
...
class TwoPointFile(object):
    ...
    def _mask_covmat(self, masks):
        ...
        if self.covmat is not None:
            ...
            self.covmat_info = CovarianceMatrixInfo(
                self.covmat_info.name,
                [s.name for s in self.spectra],
                [len(s) for s in self.spectra],
                self.covmat,
            )
    ...
    def mask_bad(self, bad_value):
        ...
        for spectrum in self.spectra:
            ...
            print(
                "Masking {} values in {}".format(mask.size - mask.sum(), spectrum.name)
            )
    ...
    def mask_cross(self):
        ...
        for spectrum in self.spectra:
            ...
            spectrum.apply_mask(mask)
            print(
                "Masking {} cross-values in {}".format(
        ...
        for spectrum in self.spectra:
            ...
            for b1, b2 in spectrum.bin_pairs:
                ...
                if (spectrum.name, b1, b2) in bin_cuts:
                    print(
                        "Removing {} bin ({},{}) altogether.".format(
                            spectrum.name, b1, b2
                        )
                    )
                    mask[w_full] = False
                ...
                if cut is None:
                    print(
                        "No cut specified for {} bin ({},{})".format(
                            spectrum.name, b1, b2
                        )
                    )
                    continue
                ...
                w = np.where(
                    (spectrum.bin1 == b1)
                    & (spectrum.bin2 == b2)
                    & ((spectrum.angle < ang_min) | (spectrum.angle > ang_max))
                )[0]

                print(
                    "Cutting {} bin pair ({},{}) to angle range ({} - {}):"
    ...
    def mask_scale(self, spectra_to_cut, min_scale=-np.inf, max_scale=np.inf):
        ...
        for spectrum in self.spectra:
            ...
            if (spectra_to_cut != "all") and (spectrum.name not in spectra_to_cut):
                ...
                spectrum.apply_mask(mask)
                print(
                    "Masking {} values in {} because they had ell or theta outside ({},{})".format(
    ...
    def choose_data_sets(self, data_sets):
        ...
        for data_set in data_sets:
            if not any(spectrum.name.lower() == data_set for spectrum in self.spectra):
                raise ValueError(
                    "Data set called {} not found in two-point data file.".format(
                        data_set
                    )
                )
    ...
    def get_cov_start(self):
        ...
        for ti, ind_i in zip(spec_names, spec_inds):
            ...
            for tj, ind_j in zip(spec_names, spec_inds):
                cov_out[
                    start_i : start_i + cov_lengths[ind_i],
                    start_j : start_j + cov_lengths[ind_j],
                ] = cov[
                    cov_starts[ind_i] : cov_ends[ind_i],
    ...
    def to_fits(self, filename, overwrite=False, clobber=False):
        ...
        if clobber:
            warnings.warn(
                "The 'clobber' keyword in twopoint is deprecated."
                "Please switch to overwrite.",
                DeprecationWarning,
            )
    ...
    def plots(
        ...
        def corr_names(spectrum):
            ...
            elif (spectrum.type1 == Types.galaxy_position_real) & (
                spectrum.type2 == Types.galaxy_shear_plus_real
            ):
                corr_type = "gt"
        ...
        if plot_spectrum:
            ...
            for spectrum in self.spectra:
                ...
                if all(bins1 == bins2):
                    ...
                    fig, ax = plt.subplots(
                        1, nbins1, figsize=(1.6 * nbins1, 1.4), sharey=True, sharex=True
                    )
                    ax = np.diag(ax)
        ...
        if plot_cov:
            ...
            figsize1 = 1.22222222227 * ncov1 / 100.0
            figsize2 = ncov2 / 100.0
...
class SpectrumCovarianceBuilder(object):
    ...
    def add_data_point(
        self, kernel1, kernel2, type1, type2, bin1, bin2, ang, angbin, value
    ):
    ...
    def generate(self, covariance, angle_unit):
        ...
        covmat_info = CovarianceMatrixInfo(
            "COVMAT",
