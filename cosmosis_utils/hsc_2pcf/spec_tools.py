...
fullsky_path = os.path.join(os.environ["COSMOSIS_SRC_DIR"], "shear/cl_to_xi_fullsky")
...
class TheorySpectrum(object):
    ...
    def from_block(cls, block, section_name):
        ...
        type_names_list, type_info_list = [t[0] for t in type_table_items], [
            t[1] for t in type_table_items
        ]
        ...
        types = (
            getattr(twopoint.Types, type_names[0]),
        ...
        return cls(
            spectrum_name, types, nbin_a, nbin_b, sep_values, spec_interps, is_auto
        )
    ...
    def get_spectrum_measurement(
        ...
        if angle_units is not None:
            ...
            angle_vals_interp = (
                angle_vals_with_units.to(twopoint.ANGULAR_UNITS["rad"])
            ).value
        else:
    ...
    def get_spec_values(self, bin1, bin2, angle):
        ...
        try:
            ...
            spec_vals[~angle_is_zero] = self.spec_interps[bin_pair](
                angle[~angle_is_zero]
            )
        return spec_vals
...
def perarcmin2_to_perrad2(n):
    n = np.array(n)
    arcmin2_per_rad2 = np.degrees(np.degrees(1.0)) * 60 * 60
...
def downsample_block(angle_lims_orig, angle_mids_orig, cov_orig, n_out):
    ...
    for i in range(n_out):
        ...
        angle_mids_out[i] = (
            weights[orig_inds_i] * angle_mids_orig[orig_inds_i]
        ).sum() / sum_w_i
        for j in range(n_out):
            ...
            cov_out[i, j] = (
                np.matmul(
                    weights[orig_inds_i],
                    np.matmul(cov_orig[cov_orig_inds_ij], weights[orig_inds_j]),
                )
                / sum_w_i
                / sum_w_j
            )
...
class ClCov(object):
    ...
    def __init__(self, theory_spectra, fsky=1.0):
    ...
    def get_cov_diag_ijkl(
        self, name1, name2, ij, kl, ell_max, ell_min=0, noise_only=False
    ):
        ...
        cl2_sum = self.get_cl2sum_ijkl(
            c_ij_12, c_kl_34, ij, kl, ell_vals, noise_only=noise_only
        )
                ...
                for i_bp, bin_pair_i in enumerate(cl_spec_i.bin_pairs):
                    for j_bp, bin_pair_j in enumerate(cl_spec_j.bin_pairs):
                        ...
                        if (i_cl == j_cl) and cl_spec_i.is_auto and (j_bp < i_bp):
                            ...
                            ell_max = ell_lims[-1]
                            cl_var_unbinned = self.get_cov_diag_ijkl(
                                cl_spec_i.name,
                                cl_spec_j.name,
                                bin_pair_i,
                                bin_pair_j,
                            ...
                            for ell_bin, (ell_low, ell_high) in enumerate(
                                zip(ell_lims[:-1], ell_lims[1:])
                            ):
                                ...
                                cl_var_binned[ell_bin] = np.sum(
                                    (2 * ell_vals_bin + 1) * cl_var_unbinned_bin
                        ...
                        # and add it there.
                        inds_i = np.arange(
                            cl_starts[i_cl] + n_ell * i_bp,
                            cl_starts[i_cl] + n_ell * (i_bp + 1),
                        )
                        inds_j = np.arange(
                            cl_starts[j_cl] + n_ell * j_bp,
...
def real_space_cov(
    ...
    if upsample is not None:
        log_angle_lims_rad_upsampled = np.linspace(
            log_angle_lims_rad[0], log_angle_lims_rad[-1], ntheta * upsample + 1
        )
        log_angle_mids_rad_upsampled = 0.5 * (
            log_angle_lims_rad_upsampled[:-1] + log_angle_lims_rad_upsampled[1:]
        )
    ...
    for i_xi in range(len(cl2xi_types)):
        ...
        F_i_l = legendre.get_F_theta_l(
            angle_mids_rad_upsampled, ell_max, cl2xi_i, high_l_filter=high_l_filter
        )
        ...
        for j_xi in range(i_xi, len(cl2xi_types)):
            ...
            if j_xi == i_xi:
                ...
                F_j_l = legendre.get_F_theta_l(
                    angle_mids_rad_upsampled,
                    ell_max,
                    cl2xi_j,
                    high_l_filter=high_l_filter,
                )
            ...
            for i_bp, bin_pair_i in enumerate(cl_spec_i.bin_pairs):
                for j_bp, bin_pair_j in enumerate(cl_spec_j.bin_pairs):
                    ...
                    if (i_xi == j_xi) and cl_spec_i.is_auto and (j_bp < i_bp):
                        ...
                        cl_cov_noise_noise = cl_cov.get_cov_diag_ijkl(
                            cl_spec_i.name,
                            cl_spec_j.name,
                            bin_pair_i,
                            bin_pair_j,
                        ...
                        xi_cov_block_signal_mixed_upsampled = np.matmul(
                            F_i_l,
                            np.matmul(np.diag(cl_cov_block_signal_mixed), F_j_l.T),
                        )
                        ...
                        xi_cov_block_noise_noise_diag = (
                            noise / angle_mids_rad / (8 * np.pi * np.pi) / dangle
                        )
                        xi_cov_block_signal_mixed, angle_mids = downsample_block(
                            angle_lims_rad_upsampled,
                            angle_mids_rad_upsampled,
                            xi_cov_block_signal_mixed_upsampled,
                            ntheta,
                        )
                        xi_cov_block = xi_cov_block_signal_mixed + np.diag(
                            xi_cov_block_noise_noise_diag
                        )
    ...
    for block_key, block_vals in cov_blocks.items():
        ...
        inds_i = np.arange(
            xi_starts[i_xi] + ntheta * i_bp, xi_starts[i_xi] + ntheta * (i_bp + 1)
        )
        inds_j = np.arange(
            xi_starts[j_xi] + ntheta * j_bp, xi_starts[j_xi] + ntheta * (j_bp + 1)
        )
        cov_inds = np.ix_(inds_i, inds_j)
